...
namespace hal::actuator {
  /**
   * @brief Dynamixel RX-64 is an exclusive smart actuator capable of reporting position, speed, and voltage. They also have the ability to set limits for angles, voltage, torque, temperature, compliance margins and slopes, and response delay.
   * 
   */
class rx_64
{
public:
  ...
  enum class register_byte : hal::byte
  {
    ...
    baud_rate = 0x04,
    /// @brief Time between sending an instruction and receiveing a status packet
    return_delay = 0x05,
    ...
    /// @brief Toggle LED
    led_toggle = 0x19,
    /// @brief Margin error between goal position and present position in the clockwise direction
    cw_compliance_margin = 0x1A,
    /// @brief Margin error between goal position and present position in the counter-clockwise direction
    ccw_compliance_margin = 0x1B,
    /// @brief Level of Torque near the goal position in clockwise direction
    cw_compliance_slope = 0x1C,
    /// @brief Level of Torque near the goal position in counter-clockwise direction
  ...
  // TODO: use different return type
  ...
  /**
   * @brief Get the temperature limit before the Overheating Error flag is set to true.
   * 
  ...
  /**
   * @brief Get the time between sending an instruction and receiveing a status packet.
   * 
   * @return uint16_t - Time in microseconds to wait before sending status packet.
   */
   ...
   * 
   * If internal temperature goes beyond this number, the Overheating Error flag is set to true. Range is 0.0 - 100.0
   * 
   ...
   * 
   * If the input voltage is below this number, the Voltage Range Error flag is set to true. Range is 5.0 - 25.0
   * 
   ...
   * 
   * If the input voltage is above this number, the Voltage Range Error flag is set to true. Range is 5.0 - 25.0
   * 
  ...
  /**
   * @brief Set the time between sending an instruction and receiveing a status packet.
   * 
